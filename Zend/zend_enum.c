...
void _enum_write_property(zval *object, zval *member, zval *value, void **cache_slot) /* {{{ */
{
	...
	zend_error(E_ERROR, "Enums cannot have dynamic properties", ce->name->val);
}
/* }}} */
...
{
	zend_error(E_ERROR, "Cannot use enum %s as array", Z_OBJCE_P(object)->name->val);
...
{
	zend_class_entry *ce = Z_OBJCE_P(object);
	zend_error(E_ERROR, "Cannot use enum %s as array", ce->name->val);
}
/* }}} */
...
{
	zend_error(E_ERROR, "Cannot refer to enum properties by reference");
	return &EG(uninitialized_zval);
}
/* }}} */
...
{
	zend_error(E_ERROR, "Cannot unset enum properties");
}
/* }}} */
...
{
	zend_error(E_ERROR, "Enum constructors cannot be called");
	return NULL;
}
/* }}} */
